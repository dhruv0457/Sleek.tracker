...
export async function DELETE(req: NextRequest) {
  ...
  const summary = `Tier:${user.tier} Trial:${user.trialEndAt?.toISOString().slice(0,10)??"none"} Habits:${user.habits.length} Checkins:${checkinCount} Badges:${user.badges.length} Trophies:${user.trophies?.count ?? 0} Reminders:${user.reminders.length} Sessions:${user.focusSessions.length} AI:${user.aiMessages.length}`;
